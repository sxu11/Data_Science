...
n_compons = [20] ## TODO: further expr
# [int(exp(x)) for x in np.linspace(log(1), log(100), num=20)]
correct_rates = []
max_rates = []
for n_compon in n_compons:
    ...
    for j1 in range(num_repeats):
        ...
        X_r = pca.fit(X).transform(X) ## TODO: by PCA
        ...
        clf = XGBClassifier() ## TODO: xgboost classifier
