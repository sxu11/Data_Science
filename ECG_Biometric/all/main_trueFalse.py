...
all_ress = []
dim_first = 9
dim_second = 90/dim_first
print dim_first, dim_second
for ind1 in range(dim_first):
    one_ress = []
    for ind2 in range(dim_second):
        method_feat = 'orig'
        method_clsf = 'DL'
        print 'method_feat: ' + method_feat + ', method_clsfy: ' + method_clsf

        ## constructing binary code
        ind_interested = ind1*dim_second + ind2 #random.sample(labels_all,1)[0]
        #print ind_interested
        labels_binary_all = [int(one_lab == ind_interested) for one_lab in labels_all]
        #print labels_all
        ##

        X_all, y_all = np.array(segs_all), np.array(labels_binary_all)
        ...
        if method_clsf == 'DL':
            ...
            res_proba = model.predict_proba(X_test)
            print ''
        else:
            if method_clsf == 'SVC':
                #print 'Method: SVC'
        ...
        if TO_PLOT_VOTES:
            i1 = 0
            for one_proba in res_proba:
                plt.plot(one_proba)
                print y_test[i1]
                #plt.scatter(y_test[i1], 0.5,'r')
                plt.show()
                i1 += 1
...
with open('all_binary_ress.txt','w') as fw:
