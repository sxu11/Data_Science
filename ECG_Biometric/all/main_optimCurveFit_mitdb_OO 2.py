...
mrks = []
mrk_locs = []
for one_anns in annss:
    mrks.append(one_anns[1])
    mrk_locs.append(one_anns[0].tolist())

annss_re = np.array(zip(mrk_locs, mrks))
######################
...
if look_data:
    ...
    plt.plot(range(look_ind), records[look_ID,:])
    mrk_locs, mrks = annss_re[look_ID,:]
    for i in range(len(mrk_locs)):
        if mrk_locs[i] >= look_ind:
            break
        plt.text(mrk_locs[i], 0, mrks[i], color='r')
    plt.show()
...
if strategy != 'NV_data':
    ...
    for i in range(len(NV_inds)):
        ...
        train_num_Vs = int(curr_num_Vs*0.1)
